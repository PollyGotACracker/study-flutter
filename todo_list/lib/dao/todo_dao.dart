...
/*
 * Future<Class>
 * 함수에서 실제 return 하는 파일 type 은 Class
 * 이 클래스를 Future 로 감싼 이유
 * 함수를 실행했을 때 실제로 내부의 코드가 작동되고
 * 어떤 데이터가 생성되는데,
 * 이 함수를 미래에 사용하여 데이터를 얻겠다 라는 선언
 * 
 * State 클래스에서 사용할 데이터를 return 하는 함수는
 * 모든 return type 을 Future<ReturnType> 로 감싸준다.
 * 그러면 함수가 실행되면서 return 되는 데이터가
 * State 를 통해서 화면에 그릴 수 있도록 한다.
 */
class TodoDao {
  // Database import: package:sqflite/sqflite.dart
...
// ''' ''' : multi-line string
  ...
  Future<Database> initDB() async {
    ...
    /**
     * join()
     * 만약 dbPath 가 '/user/db' 경로라고 하면
     * '/user/db/todo.db' 라는 이름의 실제 파일 경로를 만든다
     */
    String dbFile = join(dbPath, "todo.db");

    // dbFile 을 open 해달라
    // 만약 dbFile 이 없을 경우 onCreate 실행
  ...
  // List<TodoDto> type 으로 return 해주는 함수
  Future<List<TodoDto>> selectAll() async {
    final db = await database;
    // todo_table 에서 select 를 실행하여 todoList 에 담아라
