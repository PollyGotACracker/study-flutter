 ...
 * 
 * StateLessWidget 을 상속받아 App 이라는 클래스를 생성한다.
 * 상속받은 클래스에서는 build() 함수(메서드)를 재정의하여
...
class App extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    /*
     * flutter 로 만들어지는 화면은 가장 바깥쪽 box 에
...
/*
 * StatefulWidget
 * 변화되는 데이터, 변화되는 변수 등을 화면에 표현하기 위한 클래스
 * 이 클래스는 반드시 State<변수> 클래스를 동반한다.
 * StatefulWidget 은 데이터나 변수의 변화를 감시하고 있다가
...
/*
 * State 클래스는 변화되는 데이터, 변수 값을 화면에 그리는 역할
 * State 클래스에서 선언하는 모든 변수는 State 성질을 갖는다. 
 * 
 * 클래스, 변수, 함수 이름 앞에 underbar(_)를 붙이면
 * 모든 접근자가 private 된다.
 * 즉 다른 곳에서는 접근할 수 없고, 
 * 현재 scope 내에서만 접근 가능하도록 만드는 것
 */
class _HomePageState extends State<HomePage> {
  ...
  @override
  Widget build(BuildContext context) {
    // 부모 영역 MaterialApp 의 const 삭제(ThemeData 는 변경될 수 있는 값)
    return Scaffold(
